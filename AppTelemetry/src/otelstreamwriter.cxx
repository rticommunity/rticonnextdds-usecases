...
{
    Logger::instance().local("created StreamWriter for stream: " + _stream_name);
}

static void read_labels(const dds::core::xtypes::DynamicData& source_data, stringmap_t& labels) {
        ...
        const vector<dds::sub::SampleInfo *> &infos)
{
    ...
    for (auto sample : samples) {
        ...
        try {
            ...
            ostringstream oss;
            oss << "Received Sample: " << endl << rti::topic::to_string(*sample);
            ...
            if (sample->member_exists("labels")) {
                ...
                for (uint32_t i = 0; i < loaned_labels.get().member_count(); ++i) {                    
                    auto label_element = loaned_labels.get().loan_value(i+1);

                    // Read the key and value fields
                    std::string key = label_element.get().value<std::string>("key");
                    std::string value = label_element.get().value<std::string>("value");
            ...
            switch (discriminator) {
                case MetricTypes::UInt64Counter: {
                    ...
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("uint64_counter");
                    ...
                    auto counter = meter->CreateUInt64Counter(name, description, unit);

                    if (optional_labels.empty())
                        counter->Add(value);
                    else {
                        auto context = opentelemetry::context::Context{};
                        auto labelkv = opentelemetry::common::KeyValueIterableView<decltype(optional_labels)>{optional_labels};
                ...
                case MetricTypes::DoubleCounter: {
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("double_counter");

                    // Read the value field
                    double value = with2.value<double>("value");

                    opentelemetry::nostd::shared_ptr<metrics_api::Meter> meter = provider->GetMeter(name, "1.2.0");
                    auto counter = meter->CreateDoubleCounter(name, description, unit);
                ...
                case MetricTypes::UInt64Histogram: {
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("uint64_histogram");
                    ...
                    // RTIOsapiLibrary_openEx:!open library=liboteladapter.so: liboteladapter.so: undefined symbol: _ZNK3rti4core6xtypes15DynamicDataImpl10get_valuesImEEvRKNSt7__cxx1112basic_stringIcSt11char_traitsIcESaIcEEERSt6vectorIT_SaISD_EE
                    ...
                    opentelemetry::nostd::shared_ptr<metrics_api::Meter> meter = provider->GetMeter(name, "1.2.0");
                    auto histogram = meter->CreateUInt64Histogram(name, description, unit);
                    auto context = opentelemetry::context::Context{};
                ...
                case MetricTypes::DoubleHistogram: {
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("double_histogram");
                    ...
                    opentelemetry::nostd::shared_ptr<metrics_api::Meter> meter = provider->GetMeter(name, "1.2.0");
                    auto histogram = meter->CreateDoubleHistogram(name, description, unit);
                    auto context = opentelemetry::context::Context{};
                    auto labelkv = opentelemetry::common::KeyValueIterableView<decltype(labels)>{labels};
                    for (auto bucket: buckets) {
                        histogram->Record(bucket, labelkv, context);
                ...
                case MetricTypes::UInt64UpDownCounter: {
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("uint64_updown_counter");

                    // Read the value field
                    uint64_t value = with2.value<uint64_t>("value");

                    opentelemetry::nostd::shared_ptr<metrics_api::Meter> meter = provider->GetMeter(name, "1.2.0");
                    auto counter = meter->CreateInt64UpDownCounter(name, description, unit);
                ...
                case MetricTypes::DoubleUpDownCounter: { // DoubleUpDownCounter
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("double_updown_counter");

                    // Read the value field
                    double value = with2.value<double>("value");

                    opentelemetry::nostd::shared_ptr<metrics_api::Meter> meter = provider->GetMeter(name, "1.2.0");
                    auto counter = meter->CreateDoubleUpDownCounter(name, description, unit);
                    if (optional_labels.empty())
                        counter->Add(value);
                    else {
                        auto context = opentelemetry::context::Context{};
                ...
                case MetricTypes::Int64Gauge: { // Int64Gauge
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("int64_gauge");
                ...
                case MetricTypes::DoubleGauge: { // DoubleGauge
                    const dds::core::xtypes::DynamicData& with2 = 
                        with.value<dds::core::xtypes::DynamicData>("double_gauge");
