...
// ------------------------------------------------------------------------- //
// 
// DiscoveryListenerKind:
// This enumeration determines whether a discovery listener is being added
// to listen to Participant, DataWriter, or DataReader discovery.  Note that
// this framework currently limits the user to installing only one listener
// on the DomainParticipant, though there is no restriction in the middleware.
// ------------------------------------------------------------------------- //
enum DiscoveryListenerKind
{
	NO_DISCOVERY_KIND,
	PARTICIPANT_DISCOVERY_KIND,
	DATAWRITER_DISCOVERY_KIND,
	DATAREADER_DISCOVERY_KIND
};
...
public:
	...
	DDS::DomainParticipant* CreateParticipant(long domain, 
		DDS::DataReaderListener *discoveryListener = NULL, 
		DiscoveryListenerKind listenerKind = NO_DISCOVERY_KIND);
